...
#item_base_info_list = ['intentWeight', 'query', 'urls', 'layout', 'clicks', 'click_times', 'extraclicks', 'lds', 'hmrs', 'vfts', 'fts', 'hts', 'ans', 'exams']
item_base_info_list = ['intentWeight', 'query', 'urls', 'layout', 'clicks', 'click_times', "mouse_exam"]
...
class InputReader:
    ...
    def __call__(self, f, query_class_map):
        ...
        while True:
            ...
            layout.append(False)
            exam_predict_value_list = []
            for i in range(0, len(MOUSE_EXAM_PREDICT_MODEL)):
                tmp_ep = string_arr(arr[10 + i], " ", "float")
                exam_predict_value_list.append(tmp_ep)
            sessions.append(SessionItem(intentWeight, query, urls, layout, clicks, click_times, exam_predict_value_list))
...
class WCMouseExamUbmModel(ClickModel):
    #gammaTypesNum = 1
    def __init__(self, CLASS_K, MOUSE_EXAM_INDEX, erate, prate, ignoreIntents=True, ignoreLayout=True, explorationBias=False):
        self.explorationBias = explorationBias
        ClickModel.__init__(self, ignoreIntents, ignoreLayout)
        self.gammaTypesNum = CLASS_K
        self.mouse_exam_index = MOUSE_EXAM_INDEX
        self.erate = erate
        self.prate = prate
        ...
        for iteration_count in xrange(MAX_ITERATIONS):
            ...
            for s in sessions:
                ...
                for rank, c in enumerate(s.clicks):
                    ...
                    d = rank - prevClick - 1
                    g = self.getSumGamma(s, rank, prevClick)
                    g = g * (1 - self.erate) + s.mouse_exam[self.mouse_exam_index][rank] * self.erate
                    #if g == 0.0:
                    #    print self.mu
                    if c == 0:
                        alphaFractions[query][url][0] += a * (1 - g) / (1 - a * g)
    ...
    def _getClickProbs(self, s, possibleIntents):
        ...
        for rank, c in enumerate(s.clicks):
            ...
            for i in possibleIntents:
                a = self.alpha[s.query][url]
                g = self.getSumGamma(s, rank, prevClick)
                g = g * (1 - self.prate) + s.mouse_exam[self.mouse_exam_index][rank] * self.prate
                #g = self.getWCGamma(self.gamma, rank, prevClick)
                prevProb = 1 if rank == 0 else clickProbs[i][-1]
                if c == 0:
                    clickProbs[i].append(prevProb * (1 - a * g))
...
class WCMouseExamDbnModel(ClickModel):

    def __init__(self, gammas, MOUSE_EXAM_INDEX, erate, prate, ignoreIntents=True, ignoreLayout=True):
        self.gammas = gammas
        ClickModel.__init__(self, ignoreIntents, ignoreLayout)
        self.mouse_exam_index = MOUSE_EXAM_INDEX
        self.erate = erate
            ...
            for s in sessions:
                ...
                for intent in possibleIntents:
                    ...
                    for r in ['a', 's']:
                        positionRelevances[intent][r] = [self.urlRelevances[intent][query][url][r] for url in s.urls]
                layout = [False] * len(s.layout) if self.ignoreLayout else s.layout
                sessionEstimate = dict((intent, self._getSessionEstimate(positionRelevances[intent], layout, s.clicks, intent, s.mouse_exam[self.mouse_exam_index], self.erate)) for intent in possibleIntents)
    ...
    @staticmethod
    def getForwardBackwardEstimates(positionRelevances, gammas, layout, clicks, intent, mouse_exam, rate):
        N = len(clicks)
        if DEBUG:
            assert N + 1 == len(layout)
        alpha = [[0.0, 0.0] for i in xrange(N + 1)]
        ...
        for k, C_k in enumerate(clicks):
            ...
            #add mouse data
            if k+1 < N:
                gamma = gamma * (1 - rate) + mouse_exam[k+1] * rate #add mouse predict exam
            if gamma == 0:
                gamma += 0.000000001
            if C_k == 0:
                updateMatrix[k][0][0] = 1
    ...
    def _getSessionEstimate(self, positionRelevances, layout, clicks, intent, mouse_exam, rate):
        # Returns {'a': P(A_k | I, C, G), 's': P(S_k | I, C, G), 'C': P(C | I, G), 'clicks': P(C_k | C_1, ..., C_{k-1}, I, G)} as a dict
        # sessionEstimate[True]['a'][k] = P(A_k = 1 | I = 'Fresh', C, G), probability of A_k = 0 can be calculated as 1 - p
        N = len(clicks)
        if DEBUG:
        ...
        alpha, beta = self.getForwardBackwardEstimates(positionRelevances, self.gammas, layout, clicks, intent, mouse_exam, rate)
        try:
            varphi = [((a[0] * b[0]) / (a[0] * b[0] + a[1] * b[1]), (a[1] * b[1]) / (a[0] * b[0] + a[1] * b[1])) for a, b in zip(alpha, beta)]
        except ZeroDivisionError:
            print >>sys.stderr, alpha, beta, [(a[0] * b[0] + a[1] * b[1]) for a, b in zip(alpha, beta)], positionRelevances
                ...
                if QUERY_INDEPENDENT_PAGER:
                    for k, u in enumerate(s.urls):
                        if u == 'PAGER':
                            ...
                            positionRelevances[intent][r][k] = self.urlRelevances[intent][0][url][r]
        layout = [False] * len(s.layout) if self.ignoreLayout else s.layout
        return dict((i, self._getSessionEstimate(positionRelevances[i], layout, s.clicks, i, s.mouse_exam[self.mouse_exam_index], self.prate)['clicks']) for i in possibleIntents)

        
class WCMouseExamPositionAwareUbmModel(ClickModel):
    #gammaTypesNum = 1
    def __init__(self, CLASS_K, MOUSE_EXAM_INDEX, startRate, startStep, ignoreIntents=True, ignoreLayout=True, explorationBias=False):
        self.explorationBias = explorationBias
        ClickModel.__init__(self, ignoreIntents, ignoreLayout)
        self.gammaTypesNum = CLASS_K
        self.mouse_exam_index = MOUSE_EXAM_INDEX
    ...
    def get_model_info(self):
        ret = "++++++position aware rate\n"
        for i in range(0, MAX_DOCS_PER_QUERY):
            ret += str(1 - self.rate[i]) + "\t"
        ret += "\n+++++Gamma:\n"
        for m in range(0, self.gammaTypesNum):
            ret += "M : " + str(m) + "\n"
            for r in xrange(MAX_DOCS_PER_QUERY):
    ...
    def train(self, sessions):
        #possibleIntents = [False] if self.ignoreIntents else [False, True]# no use
        # alpha: intent -> query -> url -> "attractiveness probability"
        self.rate = [0.5 for d in xrange(MAX_DOCS_PER_QUERY)]
        self.alpha = [defaultdict(lambda: DEFAULT_REL) for q in xrange(MAX_QUERY_ID)]
        self.mu = [[(1.0 / self.gammaTypesNum) for g in xrange(self.gammaTypesNum)] for q in xrange(MAX_QUERY_ID)]
        # gamma: freshness of the current result: gammaType -> rank -> "distance from the last click" - 1 -> examination probability
        self.gamma = [[[0.5 for d in xrange(MAX_DOCS_PER_QUERY)] for r in xrange(MAX_DOCS_PER_QUERY)] for g in xrange(self.gammaTypesNum)]
        ...
        for iteration_count in xrange(MAX_ITERATIONS):
            ...
            for s in sessions:
                ...
                for rank, c in enumerate(s.clicks):
                    ...
                    #add mouse data
                    rate = self.rate[rank]
                    g = g * rate + s.mouse_exam[self.mouse_exam_index][rank] * (1-rate)
                    #if g == 0.0:
                    #    print self.mu
                    if c == 0:
                        alphaFractions[query][url][0] += a * (1 - g) / (1 - a * g)
        ...
        for s in sessions:
            ...
            for rank, c in enumerate(s.clicks):
                ...
                for i in possibleIntents:
                    ...
                    g = self.getSumGamma(s, rank, prevClick)
                    rate = self.rate[rank]
                    mouse_exam_p = s.mouse_exam[self.mouse_exam_index][rank]
                    #add mouse data
                    Er = g * rate + mouse_exam_p * (1-rate)
                    prevProb = 1 if rank == 0 else clickProbs[i][-1]
                    if c == 0:
                    ...
                    #update gradient
                    if c == 0:
                        grad[rank] += -a / (1 - a * Er) * (g - mouse_exam_p) 
                    else:
                        grad[rank] += 1 / Er * (g - mouse_exam_p)
                if c != 0:
                    prevClick = rank
...
if __name__ == '__main__':
    ...
    #data_dir = root_dir + "/cm_data/15D_click_data_click_revisit"
    data_dir = sys.argv[1]
    ...
    #sessions = readInput(data_dir + "/train_data_mouse_exam", query_class_map)
    #testSessions = readInput(data_dir + "/test_data_mouse_exam", query_class_map)
    ...
    if 'MousePositionUBM' in TEST_MODELS:
        for i in range(0, len(MOUSE_EXAM_PREDICT_MODEL)):
            model_name = 'MousePositionUBM' + "_" + MOUSE_EXAM_PREDICT_MODEL[i]
            test_model(WCMouseExamPositionAwareUbmModel(1, i, 0.7, 0.2), model_name)
    
    if 'MouseUBM' in TEST_MODELS:
        for i in range(0, len(MOUSE_EXAM_PREDICT_MODEL)):
            model_name = 'MouseUBM' + "_" + MOUSE_EXAM_PREDICT_MODEL[i]
            test_model(WCMouseExamUbmModel(1, i, 0.7, 0.7), model_name)
    
    if 'MouseDBN' in TEST_MODELS:
        for i in range(0, len(MOUSE_EXAM_PREDICT_MODEL)):
            model_name = 'MouseDBN' + "_" + MOUSE_EXAM_PREDICT_MODEL[i]
            test_model(WCMouseExamDbnModel((0.9, 0.9, 0.9, 0.9), i, 0.7, 0.7), model_name)
    
    if 'RevisitModelUBM' in TEST_MODELS or 'SQCM' in TEST_MODELS:
        test_model(RevisitModelUBM(), 'SQCM')
